...
class Packages {
	...
	public async buildTS(name? : string) {
		...
		const resolved = new Set() //编译完成的集合

		while(resolved.size !== pkgs.length) {

			for(let pkg of pkgs) { //循环所有的lib类型package

				if(resolved.has(pkg.getName())) {   //已经编译完成的就跳过
					continue
				}

				if ( !pkg.getDevLinks().find(x => !resolved.has(x))  ) { //找到未编译的进行编译
					await pkg.buildES()  //调用buildES - 执行tsc, 然后复制资源文件到es
					resolved.add(pkg.getName())
				}
			}
