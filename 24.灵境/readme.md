...
> 目前(23-12-05)，1个数据集可以上传100个文件, 单文件大小50M, 数据集的总大小为200M
...
**一旦选择关联数据集的应用，提交审核后，则不能编辑或修改当前数据集，审核完成后才可以更新**
...
官方提供的解决方案为, 使用SSE流式方式返回。（12-06：此时SSE的多包返回还不支持。）
...
> 12-06 文档要求遵循openapi规范，真实测试结果：delete接口 无法收到参数
...
* `openapi.yaml`配置的api 不宜过多，尽量做到单一职责
* `openapi.yaml`配置的api中可返回markdown语法，来优化用户体验
...
* `openapi.yaml`中的`x-return-raw`字段可以配合接口返回值，直接显示html在用户端。
  - 会出现先显示文案，后显示html的问题
  - 如果要显示超链接，需要使用markdown的语法。如果用a标签，则需要添加一个class: `chat-markdown-link`, 否则无法触发点击事件, 

* 接口返回值中，html配合prompt。（不建议使用，存在大模型自己的各种bug） 

* 接口中返回 markdown 和 html 可以共用，互补使用
...
* **提示词模版**输出格式，需要填 key 和 description 字段，key 为输出json 键值对中的键, description 为键的描述信息。
* **提问链**上由必须有一个分支链，并且分支链的意图列表中需要有一个提问链强制意图一致的意图，并且这两个一致的意图还不能相连


### GPTs插件

用户为特定目的创建的ChatGPT版本。
任何人都可以创建量身定制的GPTs，用于日常生活、特定任务、工作或家庭中获得更多便利以及提升效率，
也可以制作仅供公司内部使用的GPTs，比如帮教孩子数学或者设计贴纸、学习棋盘游戏、搜索资源、数据分析等。

### prompt创建插件

prompt创建插件是[对话平台](chat.baidu.com)中的应用制作方式。

prompt创建方式相对简单 主要是通过提示词来赋予大模型一些特定的身份和任务

灵境平台目前的开发插件类型，能力上要比这个强且复杂
...
[文心一言文档](https://yiyan.baidu.com/developer/doc#olmz2i09b)

[灵境插件](https://plugin.baidu.com/console/plugin/ability/detail/PyFLcyenlhB86yaxokAx88fEVlY13stG)

[灵境文档](https://plugin.baidu.com/docs/develop/ability-plugin/basic/develop_from_scratch/)
