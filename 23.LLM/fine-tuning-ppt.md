...
- 模型是一个数学公式，在现有的数据上使用这个公式就是训练。给定一组输入数据，经过一系列数学公式计算后，输出n个概率
...
- **损失**就是真实值和预测值之间的偏离程度，损失最小的线就是拟合度最好的线。
...
- transformer 解决了神经网络模型中的一些问题， 目前的大模型，绝大多数都是采用 transformer 架构。transformer 架构就是规定了一个特定的算法和预训练过程，大家使用这个算法和过程最终生成了一系列模型文件
...
效果取决于数据的质量，（例如微调大学信息到大模型，没有倒装句数据，那么用户使用倒装句提问，就有可能得不到答案，取决于预训练的效果）
