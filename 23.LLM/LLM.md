...
当我们把苹果拿在手里。
我们说 如果我松手，苹果就会往下掉。这叫描述
我们说 所有的东西我拿在手里，只要松手他就会往下掉，这叫猜想
我们说 往下掉的原因是因为重力，这叫解释
我们说 如果去了太空，苹果就不会往下掉了，这叫思考

智能包含的能力：描述、预测（猜想）、解释、思考

AI目前能做到的只有描述和预测，他懂的一切都来源于训练数据
...
> 1 把AI当人看。2 牢记AI的运行逻辑【通过概率预测下一个token】。 所以虽然我们可以把AI当人看，但是要记得AI只能模拟人类的行为，但是没有人类的能力。所以现在的AI只能解决question，但是不能解决problem。
...
**原则上提示词要尽量少，输入输出尽可能控制少一些， 会影响到速度和费用。**
...
- RAG场景下：不是能力问题，是大模型的数据问题。
- 微调场景下：是模型能力的问题，即使给到数据，也不一定会回答的理想。 
  - 测试能力问题：1 使用提示词，2 提示词+示例，3 微调
...
**数据和特征决定了机器学习的上限，而模型和算法只是逼近这个上限而已。所以数据的构造至关重要，需要建立对生成数据质量的把控。**
...
- 传统可复制的行业、创新行业、简单重复劳动力的行业都会被AI取代。
- 未来一定是稀缺个性化的服务、有温度的服务。
- BROKE、APE 用来作为提问模版。
...
因为现在大模型产生内容的随机性，所以目前应该是不能写好项目代码的。 风格和细节没办法在一次一次的迭代中保持稳定。
...
复杂的agent逻辑 + 快速响应【缓存、流式】

agent中需要很多验证逻辑，减少输出token，来提高速度

用大模型的理解能力，把零散的信息合并成一套信息后，交给大模型来处理。



## 一些快捷的webui工具

chatwise、cherrystudio、Chatbox、openwebui