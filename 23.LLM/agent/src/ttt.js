...
function all_list() {
  ...
  request({
    ...
    res.data.forEach(item => {
      console.log("发布时间：", new Date(item.assembleCell.itemCell.articleBase.publishTime * 1000).toLocaleString())
      console.log("标题：", item.assembleCell.itemCell.articleBase.title)
      console.log("文章ID：", item.assembleCell.itemCell.articleBase.groupID)
      console.log("文章地址1：", item.assembleCell.itemCell.articleBase.articleURL)
      console.log("文章地址2：", 'https://www.toutiao.com/article/' + item.assembleCell.itemCell.articleBase.groupID)
      console.log("更新状态：", '已更新')
      console.log("抓取时间：", '入库时创建')
      console.log('-----------------------')
...
// 草稿箱监测
function draft_list() {
  let url = "https://mp.toutiao.com/mp/agw/creator_center/draft_list?type=0&count=20&app_id=1231"
  request({
    'method': 'GET',
    'url': url,
    'headers': {
    ...
    if (error) reject(error);
    let res = JSON.parse(response.body)
    res.draft_list.forEach(item => {
      console.log("发布失败的文章标题：", item.title)
    })
  });
}
