...
9. RAG，当我们不想让大模型输出某些内容时，例如：某些黑料问题。此时可以把黑料问句放到RAG中，用来筛选到之后做过滤。

10. RAG，权限问题要注意，使用RAG的标签来进行权限控制。

11. 可以在历史对话中传入一部分role为助手的内容，这样助手就会顺着往下说。

12. 用知识库代替意图（提效）、意图之后进行知识匹配（确定知识库的内容）

13. 问题拆分、agent轮询、上下文自动识别，三种场景用来解决单个问题中多意图的场景。

14. 主动引导的问题，需要先确保引导出来的问题我们能否解决

15. 上下文的处理，知识库可以利用知识库的特性天然的处理，意图需要使用上下文分析的能力

16. 豆包的1.5pro下的角色扮演模型，可以用在一些角色扮演的场景【售前、志愿填报专家、恋爱等】

17. TOPP调整到0，可以提高输出速度
