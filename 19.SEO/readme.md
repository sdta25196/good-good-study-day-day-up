...
1. 语义：最好使用语义的 URL，这意味着它们使用单​​词而不是 ID 或随机数。
> 示例：`/learn/basics/create-nextjs-app`优于  `learn/course-1/lesson-1`
2. 逻辑一致的模式：URL 应该遵循某种在页面之间保持一致的模式。例如，您希望拥有一个将所有产品页面分组的文件夹，而不是为您拥有的每个产品设置不同的路径。
3. 以关键字为中心：爬虫希望您能在 URL 中使用关键字来帮助其理解页面的目的。
4. 不基于参数：使用参数来构建 URL 通常不是一个好主意。在大多数情况下，它们不是语义的，搜索引擎可能会混淆它们并降低它们在结果中的排名。
...
建议：
  ...
  * 使用短横杠分隔单词
  * 在前面的规则基础上尽量缩短url
  * url保持一致，不要同时拥有`/page`、`/page/`、`/page/index.html`

不建议
  * 不使用不易读的、冗长的 ID 编号 `https://www.example.com/greendress-2-3` \ `https://www.example.com/greendress?a=2995845`
  * 不要使用下划线 (_) `https://www.example.com/green_dress`
  * 不要把单词链接在一起 `https://www.example.com/greendress`

2. 向爬虫说明您的网站与链接页之间的关系
...
* **固定**的更新频率和更新周期，提高蜘蛛友好度。
* **点击率**提升排名的速度远大于**定期更新**大于**页面性能**
* 关于使用动态参数
  * 使用动态参数来渲染不同的视图，因为参数不参与爬虫的语义识别，搜索引擎可能会混淆他们并降低排名
    > 不同的视图应当使用不同的路由来渲染
  * 使用动态参数会造成url过长
    > 其实可以缩的足够短。URL层级过深、语义化不明确、过长都会影响爬虫
  * `home?a=2&b=3`和`home/2-3`渲染相似的视图时，应当配合规范标签使用前者。后者影响到了爬虫对语义的识别，且会造成大量重复页面
    > 实际上SEO不喜欢上述二者中的任意一种
